...
export default {
  ...
  methods: {
    enterDetail(id) {
      this.$router.push({ name: 'inventory-modify-detail', query: { id: id } })
    },
    openDetail() {
      this.$router.push({ name: 'inventory-modify-detail' })
    },
    async getList() {
      this.loading = true
      ...
      const {
        ...
        success
      } = await this.jaxLib.store.inventoryChangeApplyList({
        pageNo: pageIndex,
        pageSize: pageSize
      })
      this.loading = false
  ...
  <div class="inventory-modify-list">
    <div class="wh__warp">
      <div class="wh__header">
        <h2 class="wh__header--title">库存卡信息变更</h2>
      </div>
      <div class="wh__tools">
        <div class="wh__tools--mix">
          <el-button type="primary" @click="openDetail()">发起申请</el-button>
        </div>
      </div>
