    ...
    <el-card style="margin-bottom: 20px">
      <p style="font-size: 12px; margin-bottom: 30px">
        <span style="color: red">
          1、移动卡若选择本月生效，且实际【运营商侧变更确认】节点完成时间和提交时间为同一个月，则需要客户支付1元/张卡，并且此金额不计入销售提成中；</span
        ><br />
        2、电信卡只有杭州电信可以转公网限额，并且无论生效时间选择哪一个，都需要客户支付1.3元/张卡，并且此金额不计入销售提成中；<br />
        3、选定次月生效或者立即生效就不能做二次更改-运营商侧无法做二次更改;
        <br />
      ...
      >
          ...
          <el-col :span="24">
            <el-form-item label="ICCID" required prop="iccidInputType">
              ...
              >
                ...
                <el-radio v-if="formData.type === 'TW'" label="4"
                  >无，仅白名单池变更</el-radio
                >
              </el-radio-group>
            </el-form-item>
            <el-form-item
              v-if="formData.iccidInputType === '2'"
              prop="iccids"
              :rules="iccidsRules"
            >
              <el-input
            ...
            >
              <el-radio
                v-model="formData.freePlanChange"
                :label="true"
        ...
        <template v-if="formData.type === 'TW'">
          ...
          <template
            v-if="
              formData.extWhitelistWorkOrderSaveDTO.operateType === 'EXISTED'
            "
          >
            <el-form-item
              key="cmcc"
        ...
        <el-button
          ...
          type="danger"
          :loading="loading"
          @click="handleFlowClose"
          >关闭
        </el-button>
      </el-form>
    </el-card>
    <process
      v-if="flowNo && formData.status"
      :flow-no="flowNo"
      :flow-data="formData"
    />
  </div>
...
export default {
  ...
  watch: {
    "formData.freePlanChange"() {
      this.formData.priceOfferShows = this.formData.priceOfferShows.map(
        (item) => {
          return {
            ...item,
  ...
  computed: {
    ...
    computedFormRules() {
      ...
      return {
        type: [
          { required: true, message: "请选择变更类型", trigger: "submit" },
        ],
        changeReason: [
          { required: true, message: "请选择变更原因", trigger: "submit" },
        ],
        orgId: [
          { required: !isOpOrSu, message: "请选择客户ID", trigger: "submit" },
        ],
        carrier: [
          { required: true, message: "请选择运营商", trigger: "submit" },
        ],
        effectiveType: [
          { required: true, message: "请选择生效时间", trigger: "submit" },
        ],
  ...
  methods: {
    ...
    async getPoolAddress(orgId) {
      ...
      this.loading = true;
      const { success, data } = await this.jaxLib.whitelist.listPoolAddress({
        orgId,
      });
      this.loading = false;
    ...
    onTemplateChange(groupNo, isClearAddress) {
      ...
      const res =
        this.templateList.find((item) => item.groupNo === groupNo) || {};
      if (isClearAddress) {
    ...
    downloadTemplate() {
      window.open(
        "https://simboss-public.oss-cn-hangzhou.aliyuncs.com/wh/iccid-template.xlsx"
      );
    },
    removeFile() {
    ...
    async getDetail() {
      const res = await this.jaxLib.whitelist.getInfoByFlowNo({
        flowNo: this.flowNo,
      });
      if (res && res.success) {
        this.formData = {
        ...
        priceOfferShows
          ? (this.formData.priceOfferShows = safeParse(priceOfferShows).map(
              (item) => {
                return {
                  ...item,
        ...
        if (extWhitelistWorkOrderSaveDTO) {
          ...
          if (poolId) {
            this.formData.groupNo = (
              this.computedTemplateList.find(
    ...
    async queryPackagesByIccids() {
      ...
      if (res && res.success) {
        this.formData.priceOfferShows = res.data.map((item) => {
          return {
            ...item,
            ...(this.formData.priceOfferShows.find(
              (s) => s.priceOfferId === item.priceOfferId
            ) || {}),
          };
        });
      }
